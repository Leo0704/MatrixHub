...
### 5. 自动化内容流水线 (AutoCreation)

一站式自动化内容生成流水线，从产品信息输入到多平台内容输出：

**流水线阶段：**
1. **产品输入** - 输入产品名称、描述、卖点等信息
2. **内容生成** - AI 自动生成适配各平台的文案、标题
3. **预览确认** - 预览生成内容，支持手动调整
4. **批量发布** - 一键发布到多个平台账号
...
interface PipelineConfig {
  ...
  accounts: string[];        // 发布账号列表
  contentStyle: 'professional' | 'casual' | 'creative';
...
│   │   ├── pages/               # 页面组件
│   │   │   ├── Overview.tsx          # 总览
│   │   │   ├── AICreation/           # AI 创建 (含组件)
│   │   │   ├── AutoCreation/         # 自动化内容流水线 (含组件)
│   │   │   ├── ScheduledPublish.tsx  # 定时发布
│   │   │   ├── ContentManagement/    # 内容管理 (含组件)
...
│   │   ├── service-worker.ts    # 服务 Worker
│   │   ├── db.ts                # 数据库管理
│   │   ├── queue.ts             # 任务队列 (SQLite)
│   │   ├── rate-limiter.ts      # 限流器
│   │   ├── monitoring.ts        # 监控服务
│   │   ├── ai-gateway.ts        # AI 网关
...
│   │   ├── consent-manager.ts   # 用户同意管理
│   │   ├── content-moderator.ts # 内容审核
...
**使用场景：**
- AI Provider API Key 存储
- 平台登录凭证加密存储

### 内容生成流水线 (pipeline/)

自动化内容生成流水线，从产品输入到多平台内容输出。

**流水线阶段：**

1. **InputParser** - 解析产品信息输入
2. **ContentGenerator** - AI 生成多平台适配内容
3. **Orchestrator** - 流水线编排与状态管理

**状态管理：**
- 使用 SQLite 持久化流水线状态
- 支持中断恢复

### 凭证管理器 (credential-manager.ts)

统一管理平台账号凭证，集成密钥链加密存储。
...
| `task-queue.class.test.ts` | TaskQueue 类完整测试 |
...
| `ai-gateway.test.ts` | AI Gateway 请求处理 |
| `ai-director.test.ts` | AI 导演决策逻辑 |
| `account-group.test.ts` | 账号分组管理 |
| `credential-manager.test.ts` | 凭证管理 |
| `crypto-utils.test.ts` | 加密工具 |
| `consent-manager.test.ts` | 用户同意管理 |
| `strategy-engine.test.ts` | 策略引擎 |
| `keychain/index.test.ts` | 密钥链集成 |
| `pipeline/orchestrator.test.ts` | 流水线编排 |
